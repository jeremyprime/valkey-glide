...
#### Fixes
* CORE: Add per-node circuit breaker for cluster resilience. Detects unresponsive nodes via a sliding window of transport errors and rejects commands immediately, preserving throughput to healthy nodes. When tripped, drops the dead node's connection to release stuck in-flight commands. Includes exponential backoff on repeated trips and breaker-aware reconnection. ([#5861](https://github.com/valkey-io/valkey-glide/issues/5861))
